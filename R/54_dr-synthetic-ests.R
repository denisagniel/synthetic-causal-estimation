...
sim_out <- sim_res %>%
  mutate(synth_ests = Q_rows(sim_res, thisfn,n_jobs = 100, const = list(estimators = c('ate_regr', 'ate_ipw_2', 'ate_dr'))))
sim_out <- sim_out %>%
  select(-boot, -demeaned_theta) %>%
  unnest(synth_ests)
write_rds(sim_out, here('results/synthetic-ests_dr-ipw-reg.rds'))
