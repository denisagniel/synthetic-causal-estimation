...
theta1 <- read_csv(here(
  'results/comparison_sim_thetas_ld_1.csv'
)) %>%
  mutate(n = rep(c(500, 2000, 8000), each = 500*8),
         j = 1,
         d = 'ld')
mse_res <- theta1 %>%
        group_by(j, n, d, type) %>%
...
b1 <- read_csv(here(
    'results/comparison_sim_bs_ld_1.csv'
)) %>%
  mutate(n = rep(c(500, 2000, 8000), each = 500*8*4),
         j = 1,
         d = 'ld')
b_sum <- b1 %>%
...
kable(b_sum %>% arrange(n, type, est))
```
...
theta1 <- read_csv(here(
  'results/comparison_sim_thetas_ld_2.csv'
)) %>%
  mutate(n = rep(c(500, 2000, 8000), each = 500*8),
         j = 2,
         d = 'ld')
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 8}
b1 <- read_csv(here(
    'results/comparison_sim_bs_ld_2.csv'
)) %>%
  mutate(n = rep(c(500, 2000, 8000), each = 500*8*4),
         j = 2,
         d = 'ld')
